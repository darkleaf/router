...
Большинство этих проблем решены в Ruby on Rails:
...
* Предполагается построение роутинга с помощью rest ресурсов.
  ...
  Однако, фреймворк позволяет добавлять нестандартные экшены в контроллеры, что ведет к спагетти-коду.
...
* Существует апи обхода роутов, например, его использует `rake routes`.
  Также существуют сторонние библиотеки, пробрасывающие url helpers в js.
...
* Библиотека имеет одинаковый интерфейс в clojure и clojurescript,
  ...
  в виде простых структур данных с использованием кроссплатформенных шаблонов, см. пример.
